    ...
    for fc in force_col.find():
        ...
        try:
            ...
            if mem.status not in ["member", "creator", "administrator"]:
                ...
                keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("Join Channel", url=url)]])
                await message.reply(
            ...
            keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("Join Channel", url=url)]])
            await message.reply(
                f"⚠️ Gagal memeriksa keanggotaan di channel: {getattr(chat, 'title', chan_id)}",
                reply_markup=keyboard
            )
            return False
    return True
...
async def notify_restart():
    for admin_id in INITIAL_ADMINS:
        try:
            await bot.send_message(admin_id, "🔄 Bot telah di-restart dan sekarang online.")
        except:
            pass
